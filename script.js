...
// Premium Scroll Animations (Fade In Up & Styling configurations)
document.addEventListener('DOMContentLoaded', () => {
    // Inject CSS for animations and smooth scroll padding
    ...
    // Select vital elements to animate as users scroll down
    const revealElements = document.querySelectorAll('.card, .event-card, .sermon-card, .method-card, .contact-card, section h2, .schedule-item, .form-section');
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // We keep them active, unobserve isn't strictly necessary but reduces load
